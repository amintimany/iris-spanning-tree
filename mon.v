...
Section marking_definitions.
  ...
  Lemma new_marking_dom m x :
    dom (gset loc) (m ⋅ {[x := ()]}) = dom (gset loc) m ∪ {[x]}.
...
Definition of_graph_elem (G : Gmon) i v
  : option (option loc * option loc) :=
  match Gmon_graph G !! i with
  | Some w => Some w
  | None => Some v
  end.

Definition of_graph (g : graph loc) (G : Gmon) : graph loc :=
  map_imap (of_graph_elem G) g.

Definition is_marked_in (G : Gmon) x : bool :=
  if mapset_elem_of_dec x (dom (gset _) G) then true else false.
...
Definition is_marked_in_op G x w : is_marked_in (G ⋅ {[x := w]}) x = true.
Proof.
  unfold is_marked_in. destruct mapset_elem_of_dec as [Hd|Hd]; trivial.
  exfalso; apply Hd.
  rewrite dom_op elem_of_union dom_singleton elem_of_singleton; auto.
Qed.
...
Definition of_graph_empty (g : graph loc) : of_graph g ∅ = g.
Proof.
  apply: map_eq => i.
  rewrite lookup_imap /of_graph_elem lookup_omap lookup_empty //=.
  case _ : (g !! i) => //=.
Qed.
  ...
  Definition graph_inv (g : graph loc) (markings : gmap loc loc) : iProp Σ :=
    (∃ (G : Gmon) (M : markingUR),
       ...
       ★ own graph_marking_name (● M)
          ★ ([★ map] l ↦ v ∈ (of_graph g G),
               ∃ (m : loc), markings !! l = Some m
                 ★ l ↦ (#m, children_to_val v) ★ m ↦ #(is_marked_in G l))
          ★ ■ (dom (gset _) M = dom (gset _) G)
          ★ ■ (strict_subgraph g (Gmon_graph G))
    )%I.
...
Section graph_ctx_alloc.
  ...
  Lemma graph_ctx_alloc (E : coPset) (g : graph loc) (markings : gmap loc loc)
    ...
    :
      ...
      ={E}=>
      ∃ (Ig : graphG Σ) (κ : gname),
        cinv_own κ 1 ★ graph_ctx κ g markings ★ own_graph 1%Qp ∅.
  Proof.
    iIntros "H1".
    ...
    { unfold graph_inv. iExists ∅, ∅.
      ...
      - rewrite of_graph_empty {2}big_sepM_proper; eauto.
...
Lemma of_graph_dom g γ : ✓ γ → dom (gset _) (of_graph g γ) = dom (gset _) g.
Proof.
  intros H1. apply mapset_eq=>i.
  rewrite ?elem_of_dom lookup_imap. case _ : (g !! i) => [x|]; simpl; trivial.
  split; eauto. rewrite /of_graph_elem /Gmon_graph lookup_omap.
  case _ : (γ !! i) => [[]|] //=; eauto.
Qed.
...
Lemma mark_update_lookup g γ x v :
  x ∈ dom (gset _) g →
  ✓ ({[x := Excl v]} ⋅ γ) →
  of_graph g ({[x := Excl v]} ⋅ γ) !! x = Some v.
Proof.
...
Lemma mark_update_lookup_ne g γ x i v :
  i ≠ x → of_graph g ({[x := Excl v]} ⋅ γ) !! i = (of_graph g γ) !! i.
Proof.
...
Section graph.
  ...
  Lemma graph_open g markings (G : Gmon) x : x ∈ dom (gset _) g →
    own graph_name (● Some (1%Qp, G))
    ★ ([★ map] l ↦ v ∈ (of_graph g G),
         ∃ (m : loc), markings !! l = Some m ★ l ↦ (#m, children_to_val v)
           ★ m ↦ #(is_marked_in G l))
      ⊢ own graph_name (● Some (1%Qp, G))
      ★ ([★ map] l ↦ v ∈ delete x (of_graph g G),
         ∃ (m : loc), markings !! l = Some m ★ l ↦ (#m, children_to_val v)
           ★ m ↦ #(is_marked_in G l))
      ★ (∃ u, (of_graph g G !! x = Some u)
           ★ ∃ (m : loc), markings !! x = Some m ★ x ↦ (#m, children_to_val u)
               ★ m ↦ #(is_marked_in G x)).
  Proof.
    iIntros (H1) "(Hg & Ha)".
